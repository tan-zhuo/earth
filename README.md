...
作者：[tan-zhuo](https://github.com/tan-zhuo) · 博客：[tanzhuo.xyz](https://tanzhuo.xyz) · 在线体验：[www.earth.kim](https://www.earth.kim)

![演示：地球交互 → 国家详情 → 时间旅行 → 太阳系 → 宇宙](docs/demo.gif)
...
| 地形高程 / 海底地形 | [NOAA ETOPO1](https://www.ncei.noaa.gov/products/etopo-global-relief-model)（1 弧分全球地形，公有领域） | **构建时**降采样为高程贴图 → `public/textures/earth-height-rg-4096.png`、`earth-relief-4k.png`（4096×2048，RG 通道按整数米编码；空间分辨率仍为全球网格尺度） |
...
│   │   ├── terrainBorders.ts       # 贴地国界折线（合并为一次 draw call）
│   │   ├── terrainMask.ts          # 贴地高亮掩码（悬停/选中画进地表）
...
顶栏可在五级尺度间切换，每级配双语资料卡：

| 视图 | 尺度 | 内容 |
|---|---|---|
| 地球 | 10⁷ m | 完整的国家信息应用（下述全部功能） |
| 月球 | 10⁸ m | 月面贴图 + 10 个历史着陆点（阿波罗/嫦娥/月球号） |
| 太阳系 | 10¹³ m | 八大行星沿轨道公转（压缩比例），点击天体看资料，土星环、太阳光晕 |
| 银河系 | 10²¹ m | 程序化棒旋星系（5 万粒子），标注太阳系位置与人马座 A* |
| 可观测宇宙 | 10²⁷ m | 宇宙网点云（星系团 + 纤维结构），标注银河系位置 |

行星与月球贴图 © [Solar System Scope](https://www.solarsystemscope.com/textures/)（CC BY 4.0），自托管于 `public/space/`。
...
- 国旗图层：矢量国旗按原始比例展示，尺寸随镜头缩放调整，密集地区自动避让，点击国旗即选中该国；加载失败时使用高清 PNG，仍失败则显示国家代码
- 全球排行榜：GDP / 出口 / 进口（World Bank 实时数据）+ 石油 / 天然气产量（静态快照），点击榜单行直接飞到该国
...
- 地形起伏（NOAA ETOPO1）：山脉与海底地形按真实高程做**顶点位移**（球面细分到 900×450，地平线上能看出山体轮廓与遮挡）+ 坡面法线光照与山体阴影，垂直夸张 ×1~×60 可调，开启后可贴近地表观察；国界改为贴着地形走的折线，悬停/选中的高亮直接画进地表（不再是悬空的平顶盖）；「高程着色」按海拔/水深上色并附色带图例；指针悬停实时读出该点海拔/水深
